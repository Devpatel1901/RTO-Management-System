...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0-beta2/dist/js/bootstrap.bundle.min.js" integrity="sha384-b5kHyXgcpbZJO/tY9Ul7kGkf1S0CWuKcCD38l8YkeH8z8QjE0GmW1gYU5S9FOnJ0" crossorigin="anonymous"></script>
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
      ...
      <h3 style="text-align:center;margin-left:50px;margin-top:80px;font-size:45px;color:darkgreen;"><u>RTO Gujarat:Approve DL Application</u></h3>
      <br/><br/>
      ...
      <?php
              if(isset($_POST['submit']))
              {
                ...
                if ($approve == 1)
                {
                    $sql = "UPDATE DL SET dl_status = '1' , dl_issue_date = '$reg_issue' WHERE dl_id = '$id'";
                    if (mysqli_query($link,$sql))
                    {
                        echo ("<script LANGUAGE='JavaScript'>
                                window.alert('Application Approved');
                                window.location.href='view_DL.php';
                                </script>");
                    }
                    else
                    {
                ...
                {
                  $sql = "UPDATE DL SET dl_status = '-1' WHERE dl_id = '$id'";
                    if (mysqli_query($link,$sql))
                    {
                        echo ("<script LANGUAGE='JavaScript'>
